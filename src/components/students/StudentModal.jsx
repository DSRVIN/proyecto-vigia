...
import {
  ...
  Activity, Clock, Award, BarChart2, Zap, Brain, CheckCircle2, Loader2
} from 'lucide-react';
import {
      ...
      >
        ...
        <div className="sticky top-0 bg-slate-50 border-b border-slate-200 px-6 py-4 flex items-center justify-between z-10">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="font-black text-slate-900 leading-tight text-base">{student.nombre}</h2>
              <p className="text-xs text-slate-500 font-mono font-bold">{student.codigo} · {student.carrera}</p>
            </div>
          </div>
          <div className="flex items-center gap-2">
            <RiskBadge level={student.riesgo} size="sm" pulse={student.riesgo === 'CRITICO'} />
            <button onClick={onClose} className="p-1.5 rounded-lg text-slate-400 hover:text-slate-600 hover:bg-slate-200/50 transition-all">
              <X size={18} />
            </button>
          </div>
        </div>
        ...
        <div className="p-6 space-y-5">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
            {[
              { label: 'Promedio', value: promedio.toFixed(2), sub: needsRounding ? '→ 12 (visual)' : '', color: promedio >= 12 ? 'text-emerald-600' : promedio >= 10 ? 'text-amber-600' : 'text-[#d32f2f]' },
              { label: 'Nota Final', value: student.notaFinal, sub: needsRounding ? '⟳ Redondeado' : 'Sin redondeo', color: student.notaFinal >= 12 ? 'text-emerald-600' : 'text-[#d32f2f]' },
              { label: 'Asistencia', value: `${student.asistencia}%`, sub: student.asistencia < 65 ? 'Deficiente' : 'Aceptable', color: student.asistencia >= 75 ? 'text-emerald-600' : student.asistencia >= 65 ? 'text-amber-600' : 'text-[#d32f2f]' },
              { label: 'Inactividad', value: `${student.actividadDias}d`, sub: student.actividadDias > 14 ? 'Abandono Silencioso' : 'Normal', color: student.actividadDias > 21 ? 'text-[#d32f2f]' : student.actividadDias > 14 ? 'text-amber-600' : 'text-emerald-600' },
            ].map(m => (
              <div key={m.label} className="bg-slate-50 border border-slate-200 rounded-xl p-3 text-center shadow-sm">
                <p className={`text-2xl font-black ${m.color}`}>{m.value}</p>
                <p className="text-xs text-slate-500 font-bold mt-0.5">{m.label}</p>
                {m.sub && <p className={`text-[10px] mt-0.5 ${m.color} opacity-80 font-bold`}>{m.sub}</p>}
              </div>
            ))}
          </div>
            ...
            <div className="bg-slate-50 border border-slate-200 rounded-xl p-4 space-y-3 shadow-sm">
              ...
              <div className="space-y-2">
                <div className="flex justify-between text-xs font-bold">
                  ...
                  <span className={`font-black uppercase ${student.estado_pago === 'PAGADO' ? 'text-emerald-600' : 'text-orange-600'}`}>
                ...
                {student.estado_pago !== 'PAGADO' && student.detalle_pagos ? (
                  <div className="space-y-1.5 pt-1 border-t border-slate-200/50">
                    <div className="flex justify-between text-xs font-medium">
                      ...
                      <span className="font-black text-slate-800 font-mono">{student.detalle_pagos.cuotas_vencidas}</span>
                    </div>
                    <div className="flex justify-between text-xs font-medium">
                      <span className="text-slate-500">Monto Pendiente:</span>
                      <span className="font-black text-slate-800 font-mono">S/. {student.detalle_pagos.monto_pendiente.toFixed(2)}</span>
                    </div>
                    <div className="flex justify-between text-xs font-medium">
                      <span className="text-slate-500">Próximo Vencimiento:</span>
                      <span className="font-black text-slate-800 font-mono">{student.detalle_pagos.proximo_vencimiento}</span>
                    </div>
                  </div>
                ) : (
              ...
              <div className="bg-slate-50 border border-slate-200 rounded-xl p-4 space-y-3 shadow-sm">
                ...
                <div className="space-y-2.5">
                  {student.academic.cursos.map((c, idx) => (
                    <div key={idx} className="flex flex-col gap-0.5 border-b border-slate-200/60 pb-1.5 last:border-b-0 last:pb-0">
                      <div className="flex justify-between text-xs font-bold">
                        <span className="text-slate-700 truncate max-w-[170px]">{c.nombre}</span>
                        <span className={`font-mono font-black ${c.nota >= 12 ? 'text-emerald-600' : 'text-red-500'}`}>
                          {c.nota.toFixed(1)}
                        </span>
                      </div>
          ...
          <div className="bg-slate-50 border border-slate-200 rounded-xl p-4 shadow-sm">
            <h3 className="text-xs font-black text-slate-700 flex items-center gap-1.5 uppercase tracking-wider mb-4">
              <Activity size={14} className="text-blue-600" /> Evolución de Actividad en Campus Virtual
            </h3>
            <ResponsiveContainer width="100%" height={140}>
              <AreaChart data={student.actividadMensual}>
                ...
                <XAxis dataKey="mes" tick={{ fill: '#64748b', fontSize: 10, fontWeight: 600 }} axisLine={false} tickLine={false} />
          ...
          <div className="border-t border-slate-200/80 pt-4 space-y-4">
            {!showAIEditor ? (
              ...
              <div className="bg-slate-50 border border-slate-200 rounded-xl p-6 flex flex-col items-center justify-center gap-2.5 text-center shadow-inner">
                ...
                <p className="text-xs font-black text-slate-500 uppercase tracking-wider animate-pulse">Generando propuesta con Gemini 2.5 Flash...</p>
              </div>
            ) : (
          ...
          <div className="space-y-3">
            ...
            {recs.map((rec, i) => (
              <div key={i} className={`rounded-xl p-4 border shadow-sm ${rec.bg} animate-fade-in`} style={{ animationDelay: `${i * 100}ms` }}>
                <div className="flex items-center gap-1.5 mb-1.5">
                  <rec.icon size={14} className={rec.color} />
                  <span className={`text-[10px] font-black uppercase tracking-wider ${rec.color}`}>{rec.tipo}</span>
                </div>
          ...
          {!showAIEditor && (
            <div className="flex gap-2 pt-2 border-t border-slate-200/60">
              <button
                onClick={() => window.open(`mailto:${student.email || `${student.codigo.toLowerCase()}@utp.edu.pe`}?subject=Seguimiento Académico VIGÍA&body=Estimado/a ${student.nombre},%0D%0A%0D%0ALe contactamos desde el Área de Acompañamiento Académico UTP...`, '_blank')}
