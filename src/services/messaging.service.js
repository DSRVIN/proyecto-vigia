...
export async function enviarCorreoEstudiante({ student, asunto, cuerpo }) {
  ...
  console.log(
    '%c📨 [SIMULADOR CORREO INSTITUCIONAL UTP - DESPACHO EXITOSO]',
    'color: #10b981; font-weight: bold; font-size: 11px;'
  );
  console.log(
    `%cFecha/Hora: %c${new Date().toLocaleString()}`,
    'color: #94a3b8; font-weight: bold;',
    'color: #e2e8f0;'
  );
  console.log(
    `%cDestinatario: %c${student.nombre} (${student.email || `${student.codigo.toLowerCase()}@utp.edu.pe`})`,
    'color: #94a3b8; font-weight: bold;',
    'color: #e2e8f0;'
  );
  console.log(
    `%cCódigo U: %c${student.codigo} | Carrera: ${student.carrera || 'N/A'}`,
    'color: #94a3b8; font-weight: bold;',
    'color: #e2e8f0;'
  );
  console.log(
