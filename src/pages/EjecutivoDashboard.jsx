...
import {
  ...
  Percent, ArrowUpRight, ArrowDownRight, Clock,
  CheckCircle2, XCircle, ReceiptText, Filter,
...
const movimientosFinancieros = [
  { id: 'TXN-1041', fecha: '2026-07-08', tipo: 'ingreso',  categoria: 'Pensión',        monto: 18_400, descripcion: 'Pago masivo cuota Julio — 52 alumnos' },
  { id: 'TXN-1040', fecha: '2026-07-07', tipo: 'gasto',    categoria: 'Servicios',      monto: 3_850,  descripcion: 'Factura electricidad campus norte' },
  { id: 'TXN-1039', fecha: '2026-07-05', tipo: 'ingreso',  categoria: 'Pensión',        monto: 7_200,  descripcion: 'Cobro cuota atrasada — 20 alumnos' },
  { id: 'TXN-1038', fecha: '2026-07-03', tipo: 'gasto',    categoria: 'Operativo',      monto: 12_600, descripcion: 'Planilla docentes contrato parcial' },
  { id: 'TXN-1037', fecha: '2026-07-01', tipo: 'gasto',    categoria: 'Ajuste',         monto: 4_100,  descripcion: 'Redondeo contable — conciliación bancaria' },
  { id: 'TXN-1036', fecha: '2026-06-28', tipo: 'ingreso',  categoria: 'Pensión',        monto: 42_300, descripcion: 'Recaudación quincenal — 120 alumnos' },
  { id: 'TXN-1035', fecha: '2026-06-25', tipo: 'gasto',    categoria: 'Servicios',      monto: 5_200,  descripcion: 'Mantenimiento infraestructura TI' },
  { id: 'TXN-1034', fecha: '2026-06-22', tipo: 'gasto',    categoria: 'Operativo',      monto: 8_900,  descripcion: 'Materiales y suministros de aula' },
  { id: 'TXN-1033', fecha: '2026-06-20', tipo: 'ingreso',  categoria: 'Matrícula',      monto: 31_500, descripcion: 'Matrículas ciclo 2026-II — 90 alumnos' },
  { id: 'TXN-1032', fecha: '2026-06-18', tipo: 'gasto',    categoria: 'Ajuste',         monto: 2_700,  descripcion: 'Ajuste por devolución parcial ciclo anterior' },
  { id: 'TXN-1031', fecha: '2026-06-15', tipo: 'gasto',    categoria: 'Servicios',      monto: 1_980,  descripcion: 'Licencias software plataforma Canvas' },
  { id: 'TXN-1030', fecha: '2026-06-12', tipo: 'ingreso',  categoria: 'Pensión',        monto: 38_700, descripcion: 'Recaudación quincenal — 110 alumnos' },
  { id: 'TXN-1029', fecha: '2026-06-10', tipo: 'gasto',    categoria: 'Operativo',      monto: 14_200, descripcion: 'Planilla administrativa mensual' },
  { id: 'TXN-1028', fecha: '2026-06-07', tipo: 'ingreso',  categoria: 'Pensión',        monto: 9_600,  descripcion: 'Cobro tardío — 27 alumnos mora' },
  { id: 'TXN-1027', fecha: '2026-06-05', tipo: 'gasto',    categoria: 'Servicios',      monto: 6_400,  descripcion: 'Agua, gas y servicios varios' },
  { id: 'TXN-1026', fecha: '2026-06-02', tipo: 'ingreso',  categoria: 'Matrícula',      monto: 15_750, descripcion: 'Matrículas tardías ciclo 2026-II' },
  { id: 'TXN-1025', fecha: '2026-05-30', tipo: 'gasto',    categoria: 'Ajuste',         monto: 1_500,  descripcion: 'Ajuste contable fin de ciclo' },
  { id: 'TXN-1024', fecha: '2026-05-28', tipo: 'ingreso',  categoria: 'Pensión',        monto: 44_800, descripcion: 'Recaudación cierre Mayo — 127 alumnos' },
  { id: 'TXN-1023', fecha: '2026-05-25', tipo: 'gasto',    categoria: 'Operativo',      monto: 9_500,  descripcion: 'Honorarios docentes tiempo completo' },
  { id: 'TXN-1022', fecha: '2026-05-20', tipo: 'gasto',    categoria: 'Servicios',      monto: 4_300,  descripcion: 'Servicio de seguridad campus' },
];
...
const renderPieLabel = ({ cx, cy, midAngle, innerRadius, outerRadius, percent }) => {
  ...
  return (
    <text x={x} y={y} fill="white" textAnchor="middle" dominantBaseline="central"
      className="text-[10px] font-bold" fontSize={10} fontWeight={700}>
...
function KPICard({ icon: Icon, label, value, sub, trend, trendLabel, accentClass, bgClass }) {
  const isPositive = trend >= 0;
  return (
    <div className={`rounded-2xl border border-slate-200 bg-white shadow-sm p-5 flex flex-col gap-3 relative overflow-hidden`}>
      {/* Accent stripe */}
      ...
      <div className="flex items-start justify-between mt-1">
        ...
        {trend !== undefined && (
          <span className={`flex items-center gap-1 text-xs font-bold px-2 py-0.5 rounded-full ${isPositive ? 'bg-emerald-50 text-emerald-600' : 'bg-red-50 text-red-500'}`}>
...
export default function EjecutivoDashboard() {
  ...
  const totalIngresosTabla = movimientosOrdenados.filter(m => m.tipo === 'ingreso').reduce((a, m) => a + m.monto, 0);
  const totalGastosTabla   = movimientosOrdenados.filter(m => m.tipo === 'gasto').reduce((a, m) => a + m.monto, 0);
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
          <div>
            ...
            <p className="text-sm text-slate-500 mt-0.5">
              Inteligencia de Negocios · Ciclos <span className="font-bold text-slate-700">2026-I / 2026-II</span>
            </p>
          </div>
        ...
        <div className="grid grid-cols-12 gap-4">
          ...
          <div className="col-span-12 xl:col-span-8 bg-white rounded-2xl border border-slate-200 shadow-sm p-5">
            <div className="flex items-center justify-between mb-5">
              <div>
                <h2 className="text-sm font-black text-slate-800 uppercase tracking-wider">Ingresos vs. Gastos</h2>
                <p className="text-xs text-slate-400 mt-0.5">Evolución mensual Mar–Dic 2026</p>
              </div>
              <div className="flex items-center gap-4 text-xs text-slate-500">
                <span className="flex items-center gap-1.5">
                  <span className="h-2.5 w-2.5 rounded-full bg-blue-500" />Ingresos
                </span>
                <span className="flex items-center gap-1.5">
                  <span className="h-2.5 w-2.5 rounded-full bg-red-400" />Gastos
                </span>
              </div>
            </div>
            <ResponsiveContainer width="100%" height={260}>
              <AreaChart data={recaudacionMensual} margin={{ top: 4, right: 8, left: 0, bottom: 0 }}>
                <defs>
                ...
                <Area
                  ...
                  fill="url(#gradGastos)" dot={false} activeDot={{ r: 5, strokeWidth: 0 }}
                />
              </AreaChart>
            </ResponsiveContainer>
          </div>
          ...
          <div className="col-span-12 xl:col-span-4 bg-white rounded-2xl border border-slate-200 shadow-sm p-5 flex flex-col">
            <div className="mb-4">
              <h2 className="text-sm font-black text-slate-800 uppercase tracking-wider">Estado de Deuda</h2>
            ...
            <ul className="space-y-1.5 mt-2">
              {estadoDeuda.map((d) => (
                <li key={d.name} className="flex items-center justify-between text-xs">
                  ...
                  <span className="font-bold text-slate-800">{d.value.toLocaleString('es-PE')} alumnos</span>
                </li>
              ))}
            </ul>
          </div>
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-xs">
                ...
                {movimientosOrdenados.map((m, idx) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      {/* Descripción */}
                      <td className="px-4 py-3 text-slate-600 max-w-xs truncate">{m.descripcion}</td>

                      {/* Monto */}
                      <td className={`px-4 py-3 font-black tabular-nums text-right ${
                        m.tipo === 'ingreso' ? 'text-emerald-700' : 'text-red-600'
                      }`}>
                        {m.tipo === 'ingreso' ? '+' : '-'}{fmt(m.monto)}
                        {esAjuste && (
                          <span className="ml-1 text-[9px] font-normal text-slate-400 align-middle">(ajuste)</span>
                        )}
                      </td>
                    </tr>
                  );
              ...
              <tfoot className="border-t-2 border-slate-200 bg-slate-50">
                <tr>
                  <td colSpan={5} className="px-4 py-3 text-xs font-black text-slate-500 uppercase tracking-wider text-right">
                    Subtotales (filtro activo)
                  </td>
                  <td className="px-4 py-3 text-right space-y-0.5">
                    <div className="text-emerald-700 font-black text-xs">+{fmt(totalIngresosTabla)}</div>
